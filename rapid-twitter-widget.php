...
class Rapid_Twitter_Widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		//Title
		echo '<p>';
		echo '<label for="' . esc_attr( $this->get_field_id( 'title' ) ) . '">' . esc_html__( 'Title:' ) . '</label>';
		...
		//Username
		echo '<p>';
		echo '<label for="' . esc_attr( $this->get_field_id( 'account' ) ) . '">' . esc_html__( 'Twitter username:' ) . '</label>';
		echo '<input class="widefat" id="' . esc_attr( $this->get_field_id( 'account' ) ) . '" name="' . esc_attr( $this->get_field_name( 'account' ) ) . '" type="text" value="' . esc_attr( $account ) . '" />';
		echo '</p>';

		//Max Tweets
		echo '<p>';
		echo '<label for="' . esc_attr( $this->get_field_id( 'show' ) ) . '">' . esc_html__( 'Maximum number of tweets to show:' ) . '</label>';
		echo '<select id="' . esc_attr( $this->get_field_id( 'show' ) ) . '" name="' . esc_attr( $this->get_field_name( 'show' ) ) . '">';

		for ( $i = 1; $i <= 20; ++ $i ) {
			echo '<option value="' . esc_attr( $i ) . '" ' . selected( $show === $i ). '>' . esc_html( $i ) . '</option>';
		}
		...
		//Hide Replies
		echo '<p>';
		echo '<label for="' . esc_attr( $this->get_field_id( 'hidereplies' ) ) . '">';
		echo '<input id="' . esc_attr( $this->get_field_id( 'hidereplies' ) ) . '" class="checkbox" type="checkbox" name="' . esc_attr( $this->get_field_name( 'hidereplies' ) ) . '"';
		if ( $hidereplies ) {
		...
		//Include Retweets
		echo '<p>';
		echo '<label for="' . esc_attr( $this->get_field_id( 'includeretweets' ) ) . '"><input id="' . esc_attr( $this->get_field_id( 'includeretweets' ) ) . '" class="checkbox" type="checkbox" name="' . esc_attr( $this->get_field_name( 'includeretweets' ) ) . '"';
		if ( $include_retweets ) {
			echo ' checked="checked"';
		}
		...
		//Follow button
		echo '<p>';
		echo '<label for="' . esc_attr( $this->get_field_id( 'followbutton' ) ) . '"><input id="' . esc_attr( $this->get_field_id( 'followbutton' ) ) . '" class="checkbox" type="checkbox" name="' . esc_attr( $this->get_field_name( 'followbutton' ) ) . '"';
		if ( $follow_button ) {
			echo ' checked="checked"';
		}
	...
	function widget( $args, $instance ) {
		...
		$widget_ref = '';
		$widget_ref .= isset( $args['widget_id'] ) ? $args['widget_id'] : '';
		$widget_ref .= '__';
		...
		echo '}';
		echo 'if(typeof(RapidTwitter.apis.' . esc_js( $url_ref ) . ')==\'undefined\'){';
		echo 'RapidTwitter.apis.' . esc_js( $url_ref ) . '={';
		...
		echo '};';
		echo '}';
		echo 'RapidTwitter.apis.' . esc_js( $url_ref ) . '.widgets.push(\'' . esc_js( $script_id ) . '\');';
		echo '</script>';
		echo '<div id="' . esc_attr( $script_id ) . '"></div>';
		wp_enqueue_script( 'rapid-twitter-widget' );
		if ( $follow_button ) {
			wp_enqueue_style( 'rapid-twitter-widget' );
			echo '<a target="_blank" class="rapid-twitter-btn" title="Follow @' . esc_attr( $account ) . ' on Twitter" href="https://twitter.com/' . esc_attr( $account ) . '">' . "\n";
			echo '<i></i><span class="label">Follow @' . esc_attr( $account ) . '</span></a>' . "\n";
		}
...
class Rapid_Twitter_Controller {
	...
	function output_settings_page() {
		...
		<div class="wrap">
			...
			<form method="post"
			      action="<?php echo esc_url( admin_url( 'options-general.php?page=rapid-twitter-widget-settings' ) ); ?>">
				<?php
				if ( ( isset( $_REQUEST['action'] ) && 'update' === $_REQUEST['action'] ) && ( 'rapid-twitter-widget-settings' === $_REQUEST['page'] ) ) {
					$this->set_options();
				} else {
					// read the settings
					$this->get_options();
	...
	function output_key_field() {
		if ( ! defined( 'RAPID_TWITTER_KEY' ) ) {
			...
			echo esc_html( RAPID_TWITTER_KEY ) . '<br /><em>Globally defined in wp-config.php.</em>';
		}
	}
	...
	function output_secret_field() {
		if ( ! defined( 'RAPID_TWITTER_SECRET' ) ) {
			...
			echo esc_html( RAPID_TWITTER_SECRET ) . '<br /><em>Globally defined in wp-config.php.</em>';
		}
	}
	...
	function output_text_field( $id ) {
		...
		echo 'id="rapid_twitter_widget_api_' . esc_attr( $id ) . '" ';
		echo 'name="rapid_twitter_widget_api[' . esc_attr( $id ) . ']" ';
		echo 'size="30" value="' . esc_attr( $val ) . '"/>';
	}
	...
	function get_token() {
		...
		if ( $token && ( $token['signature'] !== $signature ) ) {
			unset( $token );
	...
	function get_twitter_feed( $args ) {
		...
		if ( ! $options['access_token'] || ! $options['key'] || ! $options['secret'] ) {
			...
			switch ( $response_code ) {
				case 200 : // process tweets and display
					...
					if ( ! is_array( $tweets ) || isset( $tweets['error'] ) ) {
						...
						set_site_transient( $transient_backup, $tweets, DAY_IN_SECONDS ); // A one day backup in case there is trouble talking to Twitter
					}
	...
	function ajax_rapid_twitter() {
		...
		if ( ( $args['exclude_replies'] !== 't' ) && ( $args['exclude_replies'] !== 'f' ) ) {
		...
		if ( ( $args['include_rts'] !== 't' ) && ( $args['include_rts'] !== 'f' ) ) {
			$args['include_rts'] = '';
		}
		...
		$cache_for = 5 * MINUTE_IN_SECONDS; //cache for 5 minutes
		...
		if ( $tweets !== false ) {
			...
			$jsondata = wp_json_encode( $tweets );
		} else {
			$error_msg['request'] = $reference;
			$error_msg['error']   = 'Not Found';
			$jsondata             = wp_json_encode( $error_msg );
		}
